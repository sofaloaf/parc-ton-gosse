# 🔐 Railway Environment Variables

## Backend Environment Variables

Copy these into Railway → Backend Service → Variables tab:

```
NODE_ENV=production
PORT=4000
JWT_SECRET=change-me-to-strong-secret
CORS_ORIGIN=https://your-frontend-url.railway.app
DATA_BACKEND=sheets
GS_SERVICE_ACCOUNT=parc-ton-gosse-api@parc-ton-gosse.iam.gserviceaccount.com
GS_PRIVATE_KEY="-----BEGIN PRIVATE KEY-----\\n1vcD3y7wZkWj/+OY0W76\n-----END PRIVATE KEY-----\n"
GS_SHEET_ID=1XefqX56FXtm-HVXfl9euHazaAapJDDZ4HS2EKxRV6u0
```

## Frontend Environment Variables

Copy this into Railway → Frontend Service → Variables tab:

```
VITE_API_URL=https://your-backend-url.railway.app/api
```

**Replace `your-backend-url` with your actual backend URL from Railway**

---

## ⚠️ Important Notes

1. **JWT_SECRET**: Change `change-me-to-strong-secret` to a strong random string:
   ```bash
   openssl rand -base64 32
   ```

2. **CORS_ORIGIN**: Update this AFTER you deploy the frontend and get its URL

3. **GS_PRIVATE_KEY**: Make sure to include the quotes and keep the `\n` characters

4. **VITE_API_URL**: Update this AFTER you deploy the backend and get its URL

---

## 📋 Step-by-Step

1. Deploy backend first
2. Get backend URL
3. Deploy frontend
4. Get frontend URL
5. Update `CORS_ORIGIN` in backend to frontend URL
6. Update `VITE_API_URL` in frontend to backend URL + `/api`
7. Redeploy both services

---

**Ready to deploy?** Follow the steps in `NEXT-STEPS-DEPLOYMENT.md`!

